...
<div class="mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
    ...
    <a href="<?php echo e(route('admin.users.create', ['role' => $role ?: 'customer'])); ?>"
...
<div class="grid grid-cols-3 gap-4 mb-6">
    <a href="<?php echo e(route('admin.users.index', ['role' => 'admin', 'search' => $search])); ?>"
        class="admin-filter-card group bg-white border-2 <?php echo e($role === 'admin' ? 'border-violet-400 bg-violet-50/40' : 'border-amber-300 hover:border-violet-300'); ?> rounded-2xl p-5 text-center shadow-sm transition-all">
        ...
        <p class="text-xl font-black <?php echo e($role === 'admin' ? 'text-violet-700' : 'text-slate-900'); ?>"><?php echo e($totalAdmins); ?></p>
        <p class="text-[10px] font-bold uppercase tracking-widest <?php echo e($role === 'admin' ? 'text-violet-500' : 'text-slate-400'); ?> mt-0.5">Admin</p>
    </a>
    <a href="<?php echo e(route('admin.users.index', ['role' => 'photographer', 'search' => $search])); ?>"
        class="admin-filter-card group bg-white border-2 <?php echo e($role === 'photographer' ? 'border-sky-400 bg-sky-50/40' : 'border-amber-300 hover:border-sky-300'); ?> rounded-2xl p-5 text-center shadow-sm transition-all">
        ...
        <p class="text-xl font-black <?php echo e($role === 'photographer' ? 'text-sky-700' : 'text-slate-900'); ?>"><?php echo e($totalPhotogs); ?></p>
        <p class="text-[10px] font-bold uppercase tracking-widest <?php echo e($role === 'photographer' ? 'text-sky-500' : 'text-slate-400'); ?> mt-0.5">Fotografer</p>
    </a>
    <a href="<?php echo e(route('admin.users.index', ['role' => 'customer', 'search' => $search])); ?>"
        class="admin-filter-card group bg-white border-2 <?php echo e($role === 'customer' ? 'border-amber-400 bg-amber-50/40' : 'border-amber-300 hover:border-amber-300'); ?> rounded-2xl p-5 text-center shadow-sm transition-all">
        ...
        <p class="text-xl font-black <?php echo e($role === 'customer' ? 'text-amber-700' : 'text-slate-900'); ?>"><?php echo e($totalCustomers); ?></p>
        <p class="text-[10px] font-bold uppercase tracking-widest <?php echo e($role === 'customer' ? 'text-amber-500' : 'text-slate-400'); ?> mt-0.5">Customer</p>
    </a>
</div>
...
<div class="admin-card-slate bg-white border border-amber-300 rounded-[2rem] shadow-xl shadow-slate-100/50 overflow-hidden mb-12" style="border-left: 2px solid #111827 !important; border-right: 2px solid #111827 !important;">
    ...
    <div class="p-5 sm:p-6 border-b border-slate-100 bg-slate-50/60">
        <form method="GET" action="<?php echo e(route('admin.users.index')); ?>" class="flex flex-col sm:flex-row gap-3">
            ...
            <div class="relative min-w-[12rem] flex-shrink-0">
                <select name="role" onchange="this.form.submit()"
                    ...
                    <option value="photographer" <?php echo e($role === 'photographer' ? 'selected' : ''); ?>>Fotografer</option>
    ...
    <div class="admin-meta-info px-6 flex items-center justify-between">
        <p class="text-xs font-bold text-white tracking-wide">
            Menampilkan <strong class="text-slate-900"><?php echo e($users->count()); ?></strong> user
            <?php if($search): ?> • pencarian "<strong class="text-amber-800"><?php echo e($search); ?></strong>" <?php endif; ?>
            <?php if($role): ?> • filter "<strong class="text-amber-800"><?php echo e(ucfirst($role)); ?></strong>" <?php endif; ?>
        </p>
    </div>
            ...
            <div class="overflow-hidden">
                <table class="w-full table-fixed text-xs border-collapse">
                    ...
                    <tbody>
                        ...
                        <tr class="group hover:bg-slate-50/60 transition-colors border-b border-black">
                            ...
                            <td class="py-3.5 pr-4 pl-6">
                                <div class="flex items-center gap-3">
                                    ...
                                    ?>
                                    <div class="w-8 h-8 rounded-full <?php echo e($avatarColor); ?> flex items-center justify-center flex-shrink-0 text-[10px] font-black">
                                        <?php echo e(strtoupper(substr($user->name, 0, 1))); ?>

                                    </div>
                                    <div>
                                        <p class="font-bold text-slate-900 leading-tight"><?php echo e($user->name); ?></p>
                            ...
                            <td class="py-3.5 pr-4 text-slate-900 font-medium"><?php echo e($user->email); ?></td>
                            ...
                            <td class="py-3.5 pr-4">
                                ...
                                ?>
                                <span class="inline-flex items-center px-2.5 py-1 rounded-xl border text-[9px] font-black uppercase tracking-widest <?php echo e($badgeClass); ?>">
                            ...
                            <td class="py-3.5 pr-4">
                                <?php if($user->role === 'customer'): ?>
                                    <span class="inline-flex items-center gap-1 text-amber-800 font-black text-[10px]">
                                        ...
                                        <?php echo e(number_format($user->points)); ?>
                            ...
                            <td class="py-3.5 pr-4 text-slate-900 text-[10px] font-medium"><?php echo e($user->created_at?->format('d M Y')); ?></td>

                            
                            <td class="py-3.5 text-right pr-6">
                                <div class="flex items-center justify-end gap-2">
                                    <?php if($user->role === 'customer'): ?>
                                        <button onclick="document.getElementById('points-modal-<?php echo e($user->id); ?>').showModal()"
                                    ...
                                    <?php if($user->id !== Auth::id()): ?>
                                        <form action="<?php echo e(route('admin.users.destroy', $user->id)); ?>" method="POST"
                                            onsubmit="return confirm('Hapus akun <?php echo e(addslashes($user->name)); ?>?')">
                        ...
                        <tr class="hidden">
                            <td colspan="6">
                                <dialog id="points-modal-<?php echo e($user->id); ?>"
                                    ...
                                    <div class="bg-white rounded-3xl overflow-hidden">
                                        <div class="p-6 border-b border-slate-100 bg-amber-50/50 flex items-center justify-between">
                                            <div>
                                                ...
                                                <p class="text-[10px] text-slate-500 mt-0.5">Poin saat ini: <strong class="text-amber-800"><?php echo e(number_format($user->points)); ?></strong></p>
                                            </div>
                                            <form method="dialog">
                                        ...
                                        <form action="<?php echo e(route('admin.users.points', $user->id)); ?>" method="POST" class="p-6 space-y-4">
                                            <?php echo csrf_field(); ?>
                                            <div>
...
<?php echo $__env->make('layouts.dashboard', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\asus\Documents\kuliah\smt 8\sistem informasi studio.mu\studio\resources\views/admin/users/index.blade.php ENDPATH**/ ?>