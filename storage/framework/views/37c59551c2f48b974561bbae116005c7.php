...
<div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
    ...
    <a href="<?php echo e(route('admin.bookings.index', ['status' => 'Pending', 'search' => $search, 'date_range' => $dateRange])); ?>"
        class="admin-filter-card group bg-white border-2 <?php echo e($status === 'Pending' ? 'border-amber-400 bg-amber-50/40' : 'border-amber-300 hover:border-amber-300'); ?> rounded-2xl p-4 text-center shadow-sm transition-all duration-300">
        ...
        <p class="text-xl font-black <?php echo e($status === 'Pending' ? 'text-amber-700' : 'text-slate-900'); ?>"><?php echo e($totalPending); ?></p>
        <p class="text-[9px] font-bold uppercase tracking-widest <?php echo e($status === 'Pending' ? 'text-amber-500' : 'text-slate-400'); ?> mt-0.5">Pending</p>
    </a>

    
    <a href="<?php echo e(route('admin.bookings.index', ['status' => 'Confirmed', 'search' => $search, 'date_range' => $dateRange])); ?>"
        class="admin-filter-card group bg-white border-2 <?php echo e($status === 'Confirmed' ? 'border-sky-400 bg-sky-50/40' : 'border-amber-300 hover:border-sky-300'); ?> rounded-2xl p-4 text-center shadow-sm transition-all duration-300">
        ...
        <p class="text-xl font-black <?php echo e($status === 'Confirmed' ? 'text-sky-700' : 'text-slate-900'); ?>"><?php echo e($totalConfirmed); ?></p>
        <p class="text-[9px] font-bold uppercase tracking-widest <?php echo e($status === 'Confirmed' ? 'text-sky-500' : 'text-slate-400'); ?> mt-0.5">Terkonfirmasi</p>
    </a>

    
    <a href="<?php echo e(route('admin.bookings.index', ['status' => 'Completed', 'search' => $search, 'date_range' => $dateRange])); ?>"
        class="admin-filter-card group bg-white border-2 <?php echo e($status === 'Completed' ? 'border-emerald-400 bg-emerald-50/40' : 'border-amber-300 hover:border-emerald-300'); ?> rounded-2xl p-4 text-center shadow-sm transition-all duration-300">
        ...
        <p class="text-xl font-black <?php echo e($status === 'Completed' ? 'text-emerald-700' : 'text-slate-900'); ?>"><?php echo e($totalCompleted); ?></p>
        <p class="text-[9px] font-bold uppercase tracking-widest <?php echo e($status === 'Completed' ? 'text-emerald-500' : 'text-slate-400'); ?> mt-0.5">Selesai</p>
    </a>

    
    <a href="<?php echo e(route('admin.bookings.index', ['status' => 'Cancelled', 'search' => $search, 'date_range' => $dateRange])); ?>"
        class="admin-filter-card group bg-white border-2 <?php echo e($status === 'Cancelled' ? 'border-rose-400 bg-rose-50/40' : 'border-amber-300 hover:border-rose-300'); ?> rounded-2xl p-4 text-center shadow-sm transition-all duration-300">
        ...
        <p class="text-xl font-black <?php echo e($status === 'Cancelled' ? 'text-rose-700' : 'text-slate-900'); ?>"><?php echo e($totalCancelled); ?></p>
        <p class="text-[9px] font-bold uppercase tracking-widest <?php echo e($status === 'Cancelled' ? 'text-rose-500' : 'text-slate-400'); ?> mt-0.5">Dibatalkan</p>
    </a>
</div>
...
<div class="admin-card-blue bg-white border border-amber-300 rounded-[2rem] shadow-xl shadow-slate-100/50 overflow-hidden mb-12" style="border-left: 2px solid #111827 !important; border-right: 2px solid #111827 !important;">
    ...
    <div class="p-5 sm:p-6 border-b border-slate-100 bg-slate-50/60">
        <form method="GET" action="<?php echo e(route('admin.bookings.index')); ?>" class="flex flex-col lg:flex-row items-stretch lg:items-center gap-4">
            ...
            <div class="relative flex-1">
                ...
                <input type="text" name="search" value="<?php echo e($search); ?>" placeholder="Cari ID booking, nama, email, atau layanan..."
            ...
            <div class="flex flex-col sm:flex-row items-stretch gap-3">
                ...
                <div class="relative flex-1 sm:flex-none sm:w-44">
                    <select name="status" onchange="this.form.submit()"
                        ...
                        <option value="Confirmed" <?php echo e($status === 'Confirmed' ? 'selected' : ''); ?>>Terkonfirmasi</option>
                ...
                <div class="relative flex-1 sm:flex-none sm:w-44">
                    <select name="date_range" onchange="this.form.submit()"
                        ...
                        <option value="today" <?php echo e($dateRange === 'today' ? 'selected' : ''); ?>>Hari Ini</option>
                        <option value="this_week" <?php echo e($dateRange === 'this_week' ? 'selected' : ''); ?>>Minggu Ini</option>
                        <option value="this_month" <?php echo e($dateRange === 'this_month' ? 'selected' : ''); ?>>Bulan Ini</option>
                    </select>
                    <div class="absolute inset-y-0 right-0 flex items-center pr-3.5 pointer-events-none">
                        <svg class="w-3.5 h-3.5 text-slate-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M19.5 8.25l-7.5 7.5-7.5-7.5"/></svg>
                    </div>
            ...
            <div class="flex items-center gap-2 w-full sm:w-auto mt-3 lg:mt-0 lg:ml-auto">
                ...
                <?php endif; ?>
                <a href="<?php echo e(route('admin.bookings.exportCsv', ['status' => $status, 'search' => $search, 'date_range' => $dateRange])); ?>"
                ...
                <a href="<?php echo e(route('admin.bookings.exportWord', ['status' => $status, 'search' => $search, 'date_range' => $dateRange])); ?>"
    ...
    <div class="admin-meta-info px-6">
        <p class="text-xs font-bold text-white tracking-wide">
            Menampilkan <strong class="text-slate-700"><?php echo e($bookings->count()); ?></strong> transaksi
            <?php if($search): ?> • pencarian "<strong class="text-amber-800"><?php echo e($search); ?></strong>" <?php endif; ?>
            <?php if($status): ?> 
            ...
            <?php if($dateRange): ?>
                • waktu "<strong class="text-amber-800">
                    <?php if($dateRange === 'today'): ?>
                        Hari Ini
                    <?php elseif($dateRange === 'this_week'): ?>
                        Minggu Ini
                    <?php elseif($dateRange === 'this_month'): ?>
                        Bulan Ini
                    <?php else: ?>
                ...
                <table class="w-full text-xs border-collapse">
                    ...
                    <tbody class="font-semibold text-slate-800">
                        ...
                        <tr class="group hover:bg-slate-50/60 transition-colors border-b border-black">
                            ...
                            <td class="py-4 pr-4">
                                <div>
                                    <p class="font-bold text-slate-900 text-[13px] leading-snug"><?php echo e($booking->user->name ?? 'User Terhapus'); ?></p>
                                    <p class="text-[10px] text-slate-500 font-bold tracking-wide mt-0.5"><?php echo e($booking->user->email ?? ''); ?></p>
                                </div>
                            </td>
                            ...
                            <td class="py-4 pr-4 text-slate-800 text-xs">
                                <div>
                                    <p class="font-bold text-slate-900 leading-snug"><?php echo e($booking->service_name); ?></p>
                            ...
                            <td class="py-4 pr-4 text-slate-700 text-xs font-bold">
                                <?php echo e($booking->payment_method ?? 'Transfer'); ?>

                            </td>

                            
                            <td class="py-4 pr-4 text-slate-600 font-sans text-xs">
                                <?php echo e($booking->booking_date->format('d M Y')); ?>, <?php echo e($booking->booking_date->format('H.i')); ?> - <?php echo e($booking->booking_date->copy()->addHour()->format('H.i')); ?>
                            ...
                            <td class="py-4 pr-4">
                                ...
                                <?php else: ?>
                                    <form action="<?php echo e(route('admin.bookings.assign', $booking->id)); ?>" method="POST" class="inline-block">
                                        <?php echo csrf_field(); ?>
                                        <select name="photographer_id" onchange="this.form.submit()" 
                                            ...
                                            <?php $__currentLoopData = $photographers; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $photo): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                                <option value="<?php echo e($photo->id); ?>" <?php echo e($booking->photographer_id == $photo->id ? 'selected' : ''); ?>>
                            ...
                            <td class="py-4 pr-4 text-amber-850 font-sans text-xs font-bold">
                                Rp <?php echo e(number_format($booking->amount, 0, ',', '.')); ?>
                            ...
                            <td class="py-4 pr-4">
                                ...
                                ?>
                                <span class="px-2.5 py-1 text-[9px] font-black uppercase tracking-widest rounded-full border <?php echo e($badgeStyle); ?> inline-block leading-none shadow-sm">
                            ...
                            <td class="py-4 text-right pr-6">
                                <div class="flex items-center justify-end gap-1.5">
                                    <?php if($booking->status === 'Pending'): ?>
                                        <form action="<?php echo e(route('admin.bookings.updateStatus', $booking->id)); ?>" method="POST" class="inline">
                                        ...
                                        <form action="<?php echo e(route('admin.bookings.updateStatus', $booking->id)); ?>" method="POST" class="inline">
                                            ...
                                            <button type="submit" class="px-3 py-1.5 bg-rose-500 text-white border border-rose-600 hover:bg-rose-600 font-black uppercase text-[9px] tracking-wider rounded-xl transition-all duration-300 active:scale-95 shadow-sm shadow-rose-500/30"
                                                onsubmit="return confirm('Tolak booking <?php echo e($booking->id); ?> ini?')">
                                                Tolak
                                            </button>
                                        </form>
                                    ...
                                    <button type="button"
                                        onclick="showBill('<?php echo e($booking->id); ?>', '<?php echo e(addslashes($booking->user->name ?? 'User Terhapus')); ?>', '<?php echo e($booking->user->email ?? ''); ?>', '<?php echo e(addslashes($booking->service_name)); ?>', '<?php echo e($booking->booking_date->format('d M Y')); ?>, <?php echo e($booking->booking_date->format('H.i')); ?> - <?php echo e($booking->booking_date->copy()->addHour()->format('H.i')); ?>', 'Rp <?php echo e(number_format($booking->amount, 0, ',', '.')); ?>', '<?php echo e($booking->status); ?>', '<?php echo e($booking->payment_method ?? 'Transfer'); ?>', 'Rp <?php echo e(number_format($booking->discount, 0, ',', '.')); ?>', '<?php echo e(e(json_encode($booking->addons ?? []))); ?>', '<?php echo e(addslashes($booking->review ?? '')); ?>')"
...
<?php echo $__env->make('layouts.dashboard', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\asus\Documents\kuliah\smt 8\sistem informasi studio.mu\studio\resources\views/admin/bookings/index.blade.php ENDPATH**/ ?>