                    ...
                    <td class="px-6 py-5">
                        <div class="flex items-center gap-3.5">
                            <div class="w-12 h-12 bg-amber-50 border border-amber-200 rounded-2xl flex items-center justify-center font-serif text-[11px] font-black uppercase text-amber-800 shadow-md shadow-amber-100/50 flex-shrink-0 tracking-wider leading-none p-1 text-center">
                                <?php echo e(strtoupper(substr($svc->title, 0, 3))); ?>

                            </div>
                            <div>
                                <p class="font-serif italic font-bold text-slate-900 text-[14px] leading-tight"><?php echo e($svc->title); ?></p>
                                <p class="text-[9px] font-black text-slate-500 uppercase tracking-[0.18em] mt-1"><?php echo e($svc->category); ?></p>
                            </div>
                        </div>
                    </td>
                    <td class="px-6 py-5 font-bold text-amber-800 font-sans text-xs"><?php echo e(\App\Models\Service::formatPrice($svc->starting)); ?></td>
                    <td class="px-6 py-5 text-slate-700 text-[11px] leading-relaxed max-w-xs font-semibold"><?php echo e(Str::limit($svc->description, 50)); ?></td>
                    <td class="px-6 py-5 font-semibold text-slate-800">
                        <span class="text-[9px] font-black uppercase tracking-wider block text-slate-500 mb-1"><?php echo e($svc->col1['title'] ?? ''); ?></span>
                        <span class="text-xs font-bold text-slate-900 bg-slate-50 border border-slate-200 rounded-lg px-2.5 py-1 inline-block"><?php echo e(\App\Models\Service::formatPrice($svc->col1['new'] ?? '')); ?> <span class="text-[10px] text-slate-400 font-semibold line-through ml-1">(<?php echo e(\App\Models\Service::formatPrice($svc->col1['old'] ?? '')); ?>)</span></span>
                    </td>
                    <td class="px-6 py-5 font-semibold text-slate-800">
                        <span class="text-[9px] font-black uppercase tracking-wider block text-slate-500 mb-1"><?php echo e($svc->col2['title'] ?? ''); ?></span>
                        <span class="text-xs font-bold text-slate-900 bg-slate-50 border border-slate-200 rounded-lg px-2.5 py-1 inline-block"><?php echo e(\App\Models\Service::formatPrice($svc->col2['new'] ?? '')); ?> <span class="text-[10px] text-slate-400 font-semibold line-through ml-1">(<?php echo e(\App\Models\Service::formatPrice($svc->col2['old'] ?? '')); ?>)</span></span>
                    </td>
                    <td class="px-6 py-5 text-center">
                        <div class="flex items-center justify-center gap-2">
                            <a href="<?php echo e(route('admin.services.edit', $svc->id)); ?>" class="px-4 py-2 bg-sky-500 text-white border border-transparent hover:bg-sky-600 text-[9px] font-black uppercase tracking-widest rounded-full transition-all duration-300 active:scale-95 shadow-md inline-block">Edit</a>
                            <form action="<?php echo e(route('admin.services.destroy', $svc->id)); ?>" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus layanan ini?');" class="inline-block">
                                <?php echo csrf_field(); ?>
...
<?php echo $__env->make('layouts.dashboard', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\asus\Documents\kuliah\smt 8\sistem informasi studio.mu\studio\resources\views/admin/services/index.blade.php ENDPATH**/ ?>