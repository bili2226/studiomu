<?php $__env->startSection('title', 'Loyalty & Reward'); ?>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-8 mb-12 items-stretch">
                ...
                <div class="space-y-6">
                    <div>
                        ...
                        <div class="relative rounded-2xl border border-white focus-within:border-white focus-within:ring-4 focus-within:ring-white/20 overflow-hidden flex items-center px-4 bg-transparent transition-all duration-300">
                            ...
                            <input type="number" id="setting-multiplier" value="<?php echo e($multiplier); ?>" class="w-full bg-transparent border-0 py-3.5 text-xs font-semibold focus:outline-none text-white focus:ring-0" style="background-color: transparent !important;">
                        </div>
                    </div>
                    <div>
        ...
        <div class="lg:col-span-8 admin-card-blue bg-white border border-slate-200 rounded-[2rem] shadow-xl shadow-slate-100/50 overflow-hidden flex flex-col" style="border-left: 2px solid #111827 !important; border-right: 2px solid #111827 !important;">
            <div class="flex justify-between items-center p-8 pb-6 bg-white">
                ...
                <a href="<?php echo e(route('admin.rewards.index')); ?>" class="inline-flex items-center gap-1.5 bg-amber-800 hover:bg-amber-900 text-white font-black uppercase tracking-widest text-[9px] py-2.5 px-4 rounded-xl transition-all shadow-md active:scale-95">
                ...
                <table class="w-full table-fixed text-left text-xs border-collapse">
                    ...
                    <tbody class="font-semibold text-slate-800 bg-white">
                        ...
                        <tr class="hover:bg-slate-50 transition-colors border-b border-black">
                            <td class="px-6 py-3.5"><span class="bg-slate-50 text-slate-800 border border-slate-200 rounded-lg px-2.5 py-1 text-[9px] font-black uppercase tracking-widest font-sans inline-block shadow-sm"><?php echo e($reward->code); ?></span></td>
                            <td class="px-6 py-3.5 text-slate-900"><?php echo e($reward->name); ?></td>
                            <td class="px-6 py-3.5"><span class="text-amber-800 font-extrabold font-sans text-[10px] bg-amber-50 border border-amber-200 rounded-full px-3 py-1 inline-block shadow-sm"><?php echo e($reward->points_required); ?> PTS</span></td>
...
<script>
    ...
    let loyalty = <?php echo json_encode($customers, 15, 512) ?>;
    let dbRewards = <?php echo json_encode($rewards->where('status', 'active')->values(), 512) ?>;
    let pointMultiplier = <?php echo e($multiplier); ?>;
    ...
    function saveLoyaltySettings() {
        ...
        fetch("<?php echo e(route('admin.settings.multiplier')); ?>", {
            method: 'POST',
            headers: {
...
<?php echo $__env->make('layouts.dashboard', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\asus\Documents\kuliah\smt 8\sistem informasi studio.mu\studio\resources\views/admin/loyalty/index.blade.php ENDPATH**/ ?>