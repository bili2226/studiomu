...
@section('sidebar')
    ...
    <a href="{{ route('customer.history') }}" class="sidebar-item flex items-center px-5 py-3.5 text-black hover:text-black transition-all font-bold">
        <svg class="w-4 h-4 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2.5">
    ...
    <a href="{{ route('customer.reviews') }}" class="sidebar-item sidebar-item-active flex items-center px-5 py-3.5 transition-all text-white font-black">
        <svg class="w-4 h-4 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2.5">
...
<div class="mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
    ...
    <div class="flex items-center gap-2 px-4 py-2 bg-amber-50 border border-amber-200 rounded-xl">
        ...
        <span class="text-xs font-black text-amber-800">{{ $reviews->count() }} Ulasan Diberikan</span>
    </div>
    @endif
</div>
    ...
    <div class="p-8 bg-white flex-1">
        @if ($reviews->isEmpty())
            ...
            <div class="text-center py-20 flex flex-col items-center">
                ...
                <a href="{{ route('customer.history') }}" class="mt-6 inline-flex items-center gap-2 px-5 py-2.5 bg-slate-900 hover:bg-slate-800 text-white text-[10px] font-black uppercase tracking-widest rounded-xl transition-all shadow-sm active:scale-95">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                ...
                <div class="group bg-white border-2 border-slate-100 hover:border-amber-300 rounded-2xl overflow-hidden shadow-sm hover:shadow-md hover:-translate-y-1 transition-all duration-300">
                    ...
                    <div class="bg-slate-900 px-5 py-4 flex items-center justify-between">
                        <div>
                            <span class="inline-block px-2.5 py-1 bg-amber-400/20 text-amber-400 text-[9px] font-black tracking-widest uppercase rounded-lg border border-amber-400/30">#{{ $review->id }}</span>
                        </div>
                        <div class="text-right">
                            <span class="text-[9px] text-slate-500 block">Dikirim</span>
                            <span class="text-[10px] font-black text-emerald-400 block">{{ $review->updated_at->format('d M Y') }}</span>
                        </div>
                    </div>
                    ...
                    <div class="p-5 space-y-4">
                        ...
                        <div class="border-b border-slate-100 pb-4">
                            <h3 class="font-black text-slate-900 text-sm leading-snug">{{ $review->service_name }}</h3>
                            <div class="flex flex-wrap items-center gap-3 mt-2">
                                <div class="flex items-center gap-1.5 text-[10px] text-slate-500 font-semibold">
                                    ...
                                    {{ $review->booking_date->format('d M Y, H:i') }} WIB
                                </div>
                                <div class="flex items-center gap-1.5 text-[10px] text-slate-500 font-semibold">
                                    ...
                                    {{ $review->photographer->name ?? 'Belum Ditugaskan' }}
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="relative bg-slate-50 border border-slate-200 rounded-xl p-4 group-hover:border-amber-200 group-hover:bg-amber-50/30 transition-colors duration-300">
                            ...
                            <p class="text-xs font-medium text-slate-700 italic leading-relaxed pt-1">"{{ $review->review }}"</p>
                        </div>
                        ...
                        <div class="flex items-center justify-between pt-1">
                            ...
                            <span class="text-[10px] text-slate-400 font-semibold">
                                Rp {{ number_format($review->amount, 0, ',', '.') }}
                            </span>
                        </div>
                    </div>
                </div>
