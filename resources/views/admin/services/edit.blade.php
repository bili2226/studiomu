...
<div class="bg-white border border-slate-200 rounded-[2rem] shadow-2xl shadow-slate-200/40 overflow-hidden mb-12">
    ...
    <div class="p-6 sm:p-8 bg-slate-900">
        ...
        <h2 class="text-2xl font-serif italic font-bold text-white">Edit Layanan: {{ $service->title }}</h2>
    </div>

    <form action="{{ route('admin.services.update', $service->id) }}" method="POST" enctype="multipart/form-data" class="p-6 sm:p-8 bg-slate-50/30">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
            <div>
                ...
                <input type="text" name="title" id="edit_title" value="{{ old('title', $service->title) }}" required class="w-full bg-white border border-slate-400 focus:border-slate-900 focus:ring-4 focus:ring-slate-900/5 rounded-xl px-4 py-3.5 text-xs focus:outline-none font-semibold text-slate-900 transition-all shadow-sm">
            </div>
            <div>
                <label class="block text-[10px] font-bold uppercase tracking-widest text-slate-500 mb-2.5">Harga Mulai Dari</label>
                <input type="text" name="starting" id="edit_starting" value="{{ old('starting', $service->starting) }}" required class="w-full bg-white border border-slate-400 focus:border-slate-900 focus:ring-4 focus:ring-slate-900/5 rounded-xl px-4 py-3.5 text-xs focus:outline-none font-semibold text-slate-900 transition-all shadow-sm">
            </div>
        </div>
        ...
        <div class="mb-8">
            ...
            <input id="edit_description" type="hidden" name="description" value="{{ old('description', $service->description) }}">
        ...
        <div class="mb-8">
            ...
            <input id="edit_note" type="hidden" name="note" value="{{ old('note', $service->note) }}">
        ...
        <div class="mb-8">
            ...
            <div class="flex flex-col gap-4 p-5 bg-white border border-slate-400 rounded-2xl w-full shadow-sm">
                ...
                @if(!empty($service->slides) && is_array($service->slides) && count(array_filter($service->slides)) > 0)
                <div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-8 mb-10">
            <div class="p-6 sm:p-8 bg-white border border-slate-400 rounded-[1.5rem] shadow-sm relative overflow-hidden">
                ...
                <div class="space-y-5">
                    <div>
                        ...
                        <input type="text" name="col1_title" value="{{ old('col1_title', $service->col1['title'] ?? '') }}" class="w-full bg-slate-50 border border-slate-400 focus:bg-white focus:border-slate-400 rounded-xl px-4 py-3 text-xs font-bold text-slate-800 transition-colors outline-none">
                    </div>
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            <label class="block text-[10px] font-bold uppercase tracking-wider text-slate-500 mb-2">Harga Coret</label>
                            <input type="text" name="col1_old" value="{{ old('col1_old', $service->col1['old'] ?? '') }}" placeholder="Rp 0" class="w-full bg-slate-50 border border-slate-400 focus:bg-white focus:border-slate-400 rounded-xl px-4 py-3 text-xs font-semibold text-slate-600 transition-colors outline-none line-through">
                        </div>
                        <div>
                            <label class="block text-[10px] font-bold uppercase tracking-wider text-slate-900 mb-2">Harga Baru</label>
                            <input type="text" name="col1_new" value="{{ old('col1_new', $service->col1['new'] ?? '') }}" placeholder="Rp 0" class="w-full bg-white border border-slate-300 focus:border-slate-900 rounded-xl px-4 py-3 text-xs font-bold text-slate-900 transition-colors outline-none shadow-sm">
                        </div>
                    </div>
                    <div>
                        <label class="block text-[10px] font-bold uppercase tracking-wider text-slate-500 mb-2">Fitur Paket</label>
                        <textarea name="col1_features" rows="5" class="w-full bg-slate-50 border border-slate-400 focus:bg-white focus:border-slate-400 rounded-xl px-4 py-3 text-xs font-medium text-slate-700 transition-colors outline-none" placeholder="Misal:&#10;2 Jam Sesi Foto&#10;1 Lokasi&#10;50 File Edited">{{ old('col1_features', is_array($service->col1['features'] ?? null) ? implode("\n", $service->col1['features']) : '') }}</textarea>
            ...
            <div class="p-6 sm:p-8 bg-white border border-slate-400 rounded-[1.5rem] shadow-sm relative overflow-hidden">
                ...
                <div class="space-y-5">
                    <div>
                        <label class="block text-[10px] font-bold uppercase tracking-wider text-slate-500 mb-2">Judul Paket</label>
                        <input type="text" name="col2_title" value="{{ old('col2_title', $service->col2['title'] ?? '') }}" class="w-full bg-slate-50 border border-slate-400 focus:bg-white focus:border-amber-500 focus:ring-1 focus:ring-amber-500 rounded-xl px-4 py-3 text-xs font-bold text-slate-800 transition-colors outline-none">
                    </div>
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            <label class="block text-[10px] font-bold uppercase tracking-wider text-slate-500 mb-2">Harga Coret</label>
                            <input type="text" name="col2_old" value="{{ old('col2_old', $service->col2['old'] ?? '') }}" placeholder="Rp 0" class="w-full bg-slate-50 border border-slate-400 focus:bg-white focus:border-amber-500 rounded-xl px-4 py-3 text-xs font-semibold text-slate-600 transition-colors outline-none line-through">
                        </div>
                        <div>
                            <label class="block text-[10px] font-bold uppercase tracking-wider text-amber-600 mb-2">Harga Baru</label>
                            <input type="text" name="col2_new" value="{{ old('col2_new', $service->col2['new'] ?? '') }}" placeholder="Rp 0" class="w-full bg-white border border-amber-300 focus:border-amber-500 focus:ring-1 focus:ring-amber-500 rounded-xl px-4 py-3 text-xs font-bold text-slate-900 transition-colors outline-none shadow-sm">
                        </div>
                    </div>
                    <div>
                        <label class="block text-[10px] font-bold uppercase tracking-wider text-slate-500 mb-2">Fitur Paket</label>
                        <textarea name="col2_features" rows="5" class="w-full bg-slate-50 border border-slate-400 focus:bg-white focus:border-amber-500 focus:ring-1 focus:ring-amber-500 rounded-xl px-4 py-3 text-xs font-medium text-slate-700 transition-colors outline-none" placeholder="Misal:&#10;4 Jam Sesi Foto&#10;2 Lokasi&#10;100 File Edited">{{ old('col2_features', is_array($service->col2['features'] ?? null) ? implode("\n", $service->col2['features']) : '') }}</textarea>
                        <p class="text-[9px] text-slate-400 mt-1.5 font-medium">Gunakan tombol Enter untuk memisahkan poin.</p>
                    </div>
                </div>
            </div>
